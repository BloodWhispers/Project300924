...
void AGM_FPS::HandleMatchIsWaitingToStart()
{
	...
	for(UActorComponent* comp : outComponents)
	{
		if(UGameRule* rule = Cast<UGameRule>(comp))
		{
			...
			rule->OnPointsScored.AddUniqueDynamic(this, &Handle_GameRule_PointsScored);
			_GameRulesLeft++
		}
	}
...
bool AGM_FPS::ReadyToStartMatch_Implementation()
{return false;}

bool AGM_FPS::ReadyToEndMatch_Implementation()
{return false;}
